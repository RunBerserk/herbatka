...
//!
//! Closed segments may use **trusted skip** when checkpoint lines up with sparse index stats; the tail
//! segment always replays (**tail_segment** fallback reason until then). Once any segment is replayed
//! from disk, later closed segments lose skip eligibility (**post_replay_skip_disabled**) until
//! contiguous metadata is rebuilt from replay.
...
pub(super) fn discover_segments(broker: &Broker, topic: &str) -> io::Result<Vec<SegmentMeta>> {
    segment_scan::discover_segments(broker, topic)
}

pub(super) fn load_topic_state(broker: &Broker, topic: &str) -> Result<TopicState, BrokerError> {
    load_topic::load_topic_state(broker, topic)
}
