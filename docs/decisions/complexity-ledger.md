...
| Shared broker ([`SharedBroker`](../../crates/herbatka/src/tcp/server.rs)) | **`Arc<Broker>`** with per-topic **`RwLock`** inside the map | **`Arc<Mutex<Broker>>`** or **`Arc<RwLock<Broker>>`** only | Cross-topic **Produce** / **Fetch** can overlap; same-topic produce stays ordered | Broker actor if map + per-topic locks become a bottleneck; or revert if measured overhead wins |
